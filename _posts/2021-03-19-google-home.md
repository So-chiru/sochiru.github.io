...
게다가 "스마트홈" 이라는 이름과는 달리 전혀 스마트하지 않고 자사 스피커 아니면 보이스 제어를 이용하지 못하는 심정이었죠.
...
기존에 있던 통신사에서 제공하는 스마트홈 앱을 갈아버리고 Google Home으로 연동시켜 봤습니다.
...
그러면 어떻게 기기의 패킷을 읽을 수 있을까요? 두 기기 다 손을 쓸 수 없는 상황인데.

음... 다시 간단히 생각해봅시다. 뜬금 없지만 사람 A와 사람 B가 서로 얘기를 주고 받는 상황을 가정해보죠.
...
그럴싸하게 들리는 개소리로 이상한 접근법을 얻어 냈습니다. 제 개소리가 맞다면 iOS 기기와 같은 네트워크라면 iOS 기기와 서버간의 패킷을 엿볼 수 있겠죠.
...
음... SSL은 또 뭘까요? 설명하기 위해 A와 B를 다시 부를 필요가 있겠네요.

이번엔 A와 B가 칼을 갈았습니다. 엿듣는 사람이 생기니 자기들만 알고 싶던 비밀이 더 이상 비밀이 아니게 된겁니다.
그렇게 몇날 몇일을 고민하다가 A가 기발한 수를 떠올리게 됩니다. 

A는 B에게 이렇게 말했죠, "말하는 말이 다 들린다면 우리만 아는 말로 말하면 될 거 아냐?"
...
어쨌든 이 대화 방식이 순식간에 입소문을 타 다른 사람들도 널리 쓰게되었습니다. 그리고 그 안정성을 입증한 정부에서도 사용하기 시작했죠. 이 대화법이 바로 우리가 지금 인터넷 상에서 사용하는 TLS 연결입니다.
...
이렇게 왜 우리가 내용을 알 수 없는지를 그들의 말하기 방식을 통해 상세히 알게 되었습니다. 그럼 다시 돌아가 어떻게 SSL 내용을 볼 수 있을까요? 다행히 Charles Proxy에서 SSL 내용을 볼 수 있도록 하는 기능이 존재합니다.
...
무슨 일일까요? 해당 오류는 Charles Proxy에서 SSL 연결의 내용을 읽기 위해 인증서를 바꿔치면서 최상위 인증서를 신뢰하지 못해 생긴 오류입니다. 최상위 인증서가 뭐냐구요?

아까전 이야기 기억나세요? 위 이야기의 3번을
...
아! 지금 이 상황은 브라우저가 서버에게 받은 인증서가 "최상위 인증서에 의해" 정상적으로 만들어진 인증서가 아니라서 생긴 오류입니다. 그러면 브라우저가 서버에게 받은 인증서를 정상인 것처럼 만들어야 할텐데 이를 어떻게 할까요? 간단합니다. Charles Proxy에서 만든 인증서를 정상적인 인증서로 인식할 수 있도록 Charles Proxy가 자체 서명한 최상위 인증서를 시스템이 신뢰할 수 있도록 만들면 됩니다.
...
Root CA를 설치하는 과정은 여러 곳에 알려져 있기 때문에 자세히는 설명드리지 않으나 간략하게 설명드리면 pem 파일 열고 설정에서 프로파일을 설치하고 인증서 신뢰 옵션을 키면되는 과정입니다.

이렇게 설치하면 Charles Proxy에서 SSL 패킷을 볼 수 있게 됩니다.

자, 정말 구글 홈 연동에서는 알 필요 없는 쓸모 없는 이론이 길었는데... 그냥 여기까지는 삽질이었습니다. 앱은 평범하게 통신사 서버와 통신하는 구조였고 통신사 서버에 직접 요청을 보내면 되겠으나
매번 로그인해서 세션 토큰을 가져오는 것도 귀찮고 무엇보다 연동 목적인 지연 시간 해결은 커녕 지연 시간이 더 길어질 것이 뻔하기 때문에 통신사 서버를 거치지는 않도록 하겠습니다.
...
그 선이 인터넷 단자함을 지나가는 것을 확인했고, 그 사이에 하나의 기기를 두고 오고 가는 패킷을 보겠습니다. 우선은 보안을 위해 외부 연결과는 단절된 유선 공유기를 설치해 외부에서 오는 선, 내부에서 나오는 선을 꽂고 남는 포트에 노트북을 연결합니다.

그리고 공유기 설정에 들어가 브릿지 모드를 켜고 외부 선에서 들어오는 모든 연결을 노트북에서 볼 수 있도록 설정합니다.
그러면 외부에서 들어온 모든 연결이 노트북의 포트로 들어가 패킷을 볼 수 있게되는 것이죠.

<img src="{{ '/assets/posts/2021-03-19-google-home/wireshark.png' | relative_url }}">

Charles는 HTTP 타입의 패킷만 볼 수 있기 때문에 모든 패킷을 볼 수 있는 [WireShark](https://www.wireshark.org/)를 설치하여 패킷을 관찰합니다.

<img src="{{ '/assets/posts/2021-03-19-google-home/wireshark_packets.png' | relative_url }}">

실행 후 인터페이스를 클릭하면 저렇게 무수히 많은 패킷들이 지나가는 것을 확인 할 수 있습니다. 자세한 내용은 더 적지 않겠으나, 스마트홈 애플리케이션에서 조명을 제어한 결과 특정 IP에서 특정 IP의 포트로 어느 패킷이 전송되는 것을 확인 할 수 있었습니다.

아마 발신지 IP가 관리사무소의 서버, 대상지 IP가 월패드에 부여된 IP겠죠. 대상지 IP 끝자리가 층과 호를 포함하고 있는 것을 보니 각 세대 별로 IP가 고정으로 부여되는 것 같습니다.

자세한 패킷은 나중에 분석하고, 대상지 IP를 알아냈으니 대상지 서버를 분석해보도록 하겠습니다. 우선 nmap을 사용하여 어느 포트가 열려 있는지 확인 해보도록 하죠.

```
sochiru@arch:~$ sudo nmap -sS (대상지 IP 주소) -p 1-65535
...
???? 분명히 프로덕션 제품일텐데도 ssh 포트가 열려 있네요. 개발 단계에서 디버깅을 위해 만든 포트 같습니다. 그 외에는 관리 사무소와 통신, 디버깅용 포트네요.
바로 ssh 연결 날려봤습니다.
...
루트 비밀번호가 너무 쉽습니다. Bruteforce나 수많은 Dictionary Attack 없이도 "인간이라면 넣어볼만한 사전 공격"을 통해 비밀번호를 쉽게 따냈습니다.

루트 비밀번호를 어렵게 설정하든, 디버깅 포트를 비활성화하든 대책이 필요해 보입니다.
...
얼떨결에 월패드의 바이너리 파일과 시스템 암호를 취득했는데, 바이너리 분석은 아직 흥미와 지식이 없어 다음에 기회가 된다면 그 때 분석을 해봐야겠습니다.

어쨋든, ssh 세션을 종료하고 다시 패킷 분석으로 돌아가 기기를 누르면서 각 패킷을 보니 다음과 같은 형식으로 TCP 패킷이 전송되더군요.
...
앞의 8글자는 본문의 길이. 그 후로는 본문을 담아 전송하는 매우 간단한 형태의 패킷입니다. 어디서 전송되는 내용인지는 따로 구별하지 않는 듯 싶습니다.
